...
public class Chat extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(((String)request.getParameter("action")).equals("updateSeenStatus")){
			...
			String messages = "<input type='hidden' id='part' value='"+part+"'/> <input type='hidden' id='disId' value='"+id+"'/>";
			if(chat.length()>0){
				for (int i = 0; i < chat.length(); i++) {
					JSONObject rec = chat.getJSONObject(i);
					if(rec.getBoolean("isByCreator")){
						if(((String)request.getParameter("action")).equals("edit")){
							messages = messages+"<div class='fromMe'><span style='float: left; width: 75%; margin-left: 5px;text-align: right;'>"+rec.getString("message")+"</span><span class='me'>: Me</span></div>";
						}else{
							messages = messages+"<div class='toMe'><span style='width: 75%;text-align: left;'>"+rec.getString("message")+"</span></div>";
						}
					}else{
						if(((String)request.getParameter("action")).equals("edit")){
							messages = messages+"<div class='toMe'><span style='width: 75%;text-align: left;'>"+rec.getString("message")+"</span></div>";
	...
	public void saveChat(int id,String part,String message,boolean isByCreator){
		...
		try {
			...
			String query = "insert into tbl_chat(dis_id,part,message,isByCreator) values("+id+",'"+part+"','"+message+"',"+byCreator+")";
	...
	public JSONArray getChat(int id,String part,int isByCreator) throws SQLException {
		...
		String query = "Select * from tbl_chat where dis_id = '" + id
				+ "' and part = '"+part+"'";
	...
	public void updateSeenStatus(int id,String part,int isByCreator) throws SQLException{
		Connection conn = pool.getConnection();
		Statement stmt = conn.createStatement();
		String query = "update tbl_chat set isSeen = 1 where dis_id = '" + id
				+ "' and part = '"+part+"' and isByCreator = '"+isByCreator+"'";
	...
	public JSONArray getAllChat(int isCreator) throws SQLException{
		...
		//"Select (select COUNT(*) FROM tbl_chat where part = 'sherryyang12321@gmail.com' and isByCreator = 1 and isSeen = 0 and tbl_chat.dis_id = tbl_discussion.dis_id) as isSeen,tbl_chat.id,tbl_chat.dis_id,tbl_chat.part,tbl_discussion.dis_title from tbl_chat inner join tbl_discussion on tbl_discussion.dis_id = tbl_chat.dis_id where part = 'sherryyang12321@gmail.com' group by tbl_chat.dis_id"
		String query = "Select (select COUNT(*) FROM tbl_chat where part = '"+user+"' and isByCreator = '"+isCreator+"' and isSeen = 0 and tbl_chat.dis_id = tbl_discussion.dis_id) as isSeen,tbl_chat.dis_id,tbl_chat.isSeen,tbl_chat.part,tbl_discussion.dis_title from tbl_chat inner join tbl_discussion on tbl_chat.dis_id = tbl_discussion.dis_id where part = '"+user+"' group by tbl_chat.dis_id";
